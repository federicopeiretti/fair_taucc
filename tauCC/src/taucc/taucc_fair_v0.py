# Fair TauCC (modello di partenza)
    ...
    * Battaglia E., et al., 2023. `Fast parameterless prototype-based co-clustering`
    ...
    def _init_all(self, V, Sx):
        ...
        if isinstance(Sx, np.ndarray) and Sx.shape[0] == self._n_documents:
            ...
            self.Sx = Sx
            # length of Sx
            self.Sx_n_rows = self.Sx.shape[0]
            # protected groups
            self.Sx_groups = dict()
            for group in np.unique(self.Sx):
                self.Sx_groups[group] = np.where(self.Sx == group)[0]
            self.Sx_n_groups = np.unique(self.Sx).shape[0]
        else:
